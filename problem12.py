...
def elliptic_smooth(X_init, Y_init, dS, iterations=500):
    ...
    for iter in range(iterations):
        ...
        P_final = np.copy(P_bg)
        Q_final = np.copy(Q_bg)
        decay = 0.35 
...
for j in range(M + 1):
    ax1.plot(X_e[:, j], Y_e[:, j], 'k-', lw=0.15)
for i in range(N + 1):
    ax1.plot(X_e[i, :], Y_e[i, :], 'k-', lw=0.15)
ax1.set_aspect('equal')

for j in range(30):
    ax2.plot(X_e[:, j], Y_e[:, j], 'k-', lw=0.3)
for i in range(N + 1):
    ax2.plot(X_e[i, :30], Y_e[i, :30], 'k-', lw=0.3)
